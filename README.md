...
| 2 | Full walking policy (domain randomization for robustness) | Needs retraining — a joint-range/rest-pose bug fix changed the action space, see `mujoco_rl/README.md` |
| 3 | Small-stair climbing (procedural terrain, blind proprioceptive policy) | Needs retraining, same reason — see `mujoco_rl/README.md` for the failure modes found and fixed so far |
